 ...
 * @param {{fractions?: number[]}} [opts]
 */
export async function analyzeTruePeakAndClipping(left, right, sampleRate, onProgress, opts) {
  ...
  const frac = Array.isArray(opts?.fractions) && opts.fractions.length
    ? opts.fractions.filter((x) => typeof x === 'number' && Number.isFinite(x) && x > 0 && x < 1)
    : [0.25, 0.5, 0.75];
  ...
  // Avoid reporting 100% here: there is still packing/serialization work after TruePeak,
  // and in compare-mode we may still analyze the reference file.
  onProgress?.({ stage: 'TruePeak', detail: `finalizacja…` });
