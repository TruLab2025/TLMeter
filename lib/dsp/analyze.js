...
const DEFAULT_ANALYSIS_WINDOW_SEC = 30;
...
function createEffectiveOptions(options) {
  ...
  return {
    ...
    analysisWindowSec: (typeof base.analysisWindowSec === 'number' && Number.isFinite(base.analysisWindowSec))
      ? Math.max(5, Math.min(60, base.analysisWindowSec))
      : DEFAULT_ANALYSIS_WINDOW_SEC,
    truePeakFractions: Array.isArray(base.truePeakFractions) ? base.truePeakFractions : undefined,
  };
}

async function findLoudestWindowStart(left, right, sampleRate, windowSec, options) {
  ...
  for (let i = 0; i < n; i += stride) {
    ...
    if ((i & 0x3ffff) === 0) {
      ...
      if (now - lastYield > 18) {
        if (options?.isAborted && options.isAborted()) throw new Error("Analiza została anulowana.");
        options?.onProgress?.({ stage: 'Window', detail: `skanuję plik… ${Math.round((i / n) * 100)}%` });
        // Let worker/UI breathe
...
async function createFocusedAudioBuffer(audioBuffer, options) {
  const sampleRate = audioBuffer.sampleRate;
  const channels = audioBuffer.numberOfChannels;
  const durationSec = audioBuffer.duration;
  const focusSec = options?.analysisWindowSec ?? DEFAULT_ANALYSIS_WINDOW_SEC;
  ...
  const startSample = await findLoudestWindowStart(left, right, sampleRate, focusSec, options);
 ...
 * @param {{frameMs:number, hopMs:number, rolloffPercent:number, onProgress?:(p:any)=>void, forceEssentia?: boolean}} options
 */
export async function analyzeAudioBuffer(audioBuffer, options) {
      // (usunięto nieprawidłowe użycie essentiaBundle przed deklaracją)
    console.log('[ANALYZE] START', {
      forceEssentia: options?.forceEssentia,
      frameMs: options?.frameMs,
      hopMs: options?.hopMs
    });
  const effectiveOptions = createEffectiveOptions(options);

  // Wymuś Essentia jeśli forceEssentia (np. Premium)
  if (options?.forceEssentia) {
    effectiveOptions.forceEssentia = true;
  }
  const focused = await createFocusedAudioBuffer(audioBuffer, effectiveOptions);
  ...
  if (essentiaBundle && effectiveOptions.forceEssentia) {
    console.log('[ANALYZE] Używam Essentia (WASM) – wymuszone przez Premium, frameMs:', effectiveOptions.frameMs, 'hopMs:', effectiveOptions.hopMs);
  ...
  // Równoległa analiza: spectrum, Meyda, spectralAdv, HPSS
  ...
  const [
    ...
    (async () => {
      ...
      try {
        ...
        await new Promise(r => setTimeout(r, 0));
        return await analyzeMeydaFeatures(left, right, sampleRate, { frameSize, hopSize, onProgress: effectiveOptions.onProgress });
      } catch (e) {
    ...
    (async () => {
      ...
      try {
        ...
        if (count > 0) {
          ...
          return { slope: 0, entropy: 0, crestPerBand: { low: 0, mid: 0, high: 0 }, lowMid: { fraction: 0, percent: 0 } };
        }
      } catch (e) {
        return { error: String(e) };
      }
    ...
    (async () => {
      ...
      try {
        if (effectiveOptions.isAborted?.()) throw new Error("Analiza została anulowana.");
        await new Promise(r => setTimeout(r, 0));
        return await analyzeHarmonicPercussive(monoMix, sampleRate, { frameSize, hopSize, onProgress: effectiveOptions.onProgress });
      } catch (e) {
        return { error: String(e) };
      }
    })()
  ...
  // Pozostałe kroki zależne od spectrum
  ...
  const truePeak = await analyzeTruePeakAndClipping(left, right, sampleRate, effectiveOptions.onProgress, {
    fractions: effectiveOptions.truePeakFractions,
  });
  effectiveOptions.onProgress?.({ stage: 'Pakowanie', detail: 'Składam wynik…' });
  await yieldMs(60, effectiveOptions);
  ...
  console.log('[ANALYZE] KONIEC', {
    czasMs: Math.round(elapsed),
    frameMs: effectiveOptions.frameMs,
    hopMs: effectiveOptions.hopMs,
    forceEssentia: !!effectiveOptions.forceEssentia
  });
}
